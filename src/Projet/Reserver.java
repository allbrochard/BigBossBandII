...
public class Reserver {
	...
	public boolean reserverSalle (){
		...
		boolean res = false;
		String query = "INSERT INTO public.reservation (dateresa, idmatierefk, idformateurfk, idpromofk, idsallefk) VALUES (?,(SELECT idmatiere FROM public.matiere WHERE idmatiere = ?),(SELECT idpromo FROM public.promo WHERE idpromo = ?),(SELECT idsalle FROM public.salles WHERE idsalle = ?),(SELECT idformateur FROM public.formateur WHERE idformateur = ?)) RETURNING idresa;";
		try {
